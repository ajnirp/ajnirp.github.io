...
A majority element occurs in more than half the indices in an array. So if an array ahs $n$ elements and $k$ of them are the majority elements, then $\frac{k}{n} \ge \frac{1}{2}$.
...
$\frac{k-1}{n-2} \geq \frac{k}{n}\\\
=> kn - n \geq kn - 2k\\\
=> 2k \geq n\\\
=> \frac{k}{n} \geq \frac{1}{2}$

The $\geq$ can be replaced by a $\e$, and we're done. The new ratio is greater than the old one (regardless of what $k$ and $n$ were), and the old one was greater than $1/2$. So the former majority element is the new majority element!
