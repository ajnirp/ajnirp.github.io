...
The final step above was me expanding out all the brackets and pushing everything over to the left. I also kept the parens around the last two terms on the left because I wanted to make the equation look like — you guessed it — a quadratic equation in {{ katex(body="p" )}}.
...
For the numerator to be an integer, the quantity {{ katex(body="1 + 12(4h^2 - 2h)") }} must be a perfect square. That's what the `int(s) == s` check is for. The rest is pretty straightforward — once we find a value for {{ katex(body="h") }} that yields an integer value for {{ katex(body="p") }}, we find the corresponding value of {{ katex(body="t") }}, print the values and then break out of our otherwise-infinite `while` loop.
